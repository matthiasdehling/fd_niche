...
inhull <- function(testpts, calpts, hull=convhulln(calpts), tol=mean(mean(abs(calpts)))*sqrt(.Machine$double.eps)) {
  # R implementation of the Matlab code by John D'Errico 04 Mar 2006 (Updated 30 Oct 2006) with some modifications and simplifications
  ...
  #% calpts - mxp array of vertices of the convex hull, as used by convhulln.
  ...
  #% tol - (OPTIONAL) tolerance on the tests for inclusion in the convex hull. 
  #  You can think of tol as the distance a point may possibly lie outside the hull, and still be perceived 
  #% as on the surface of the hull. Because of numerical slop nothing can ever be done exactly here. I might guess a
  #% semi-intelligent value of tol to be tol = 1.e-13*mean(abs(calpts(:)))
